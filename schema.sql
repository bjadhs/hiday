...
-- KProjects
-- ============================================================
-- Must exist before `sessions` (which references it via kproject_id).
CREATE TABLE IF NOT EXISTS public.kprojects (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
...
ALTER TABLE public.kprojects ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can CRUD their own kprojects"
  ON public.kprojects
  FOR ALL
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
...
CREATE TABLE IF NOT EXISTS public.sessions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  -- Nullable: inbox todos can exist before a project is assigned.
  project_id UUID REFERENCES public.projects(id) ON DELETE CASCADE,
  kproject_id UUID REFERENCES public.kprojects(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_sessions_kproject_id ON public.sessions(kproject_id);
...
CREATE INDEX IF NOT EXISTS idx_kprojects_user_id ON public.kprojects(user_id);
