...
import { PROJECT_ICONS, PROJECT_COLORS } from '@/lib/constant';
import { EditableProject } from '../types';
import { cn } from '@/lib/utils';
import { projectInputSchema, formatZodErrors } from '@/lib/validation';
import {
...
export function ProjectFormModal({
    ...
    const [formData, setFormData] = useState<EditableProject>({
        ...project,
        project_tags: project.project_tags || [],
        goal_type: project.goal_type || 'none',
        note_prompt: project.note_prompt ?? false,
        default_note: project.default_note || '',
    });
                ...
                <div className='p-6 border-b-4 border-border-strong bg-muted/10 flex items-center justify-between'>
                    <div className='space-y-1'>
                        <DialogTitle className='text-3xl font-black tracking-tighter'>
                            {isNew ? 'CREATE PROJECT' : 'REFINE PROJECT'}
                        </DialogTitle>
                        <DialogDescription className='sr-only'>
                            {isNew ? 'Create a new project blueprint' : 'Refine your existing project configuration'}
                        </DialogDescription>
                    </div>
                    <button
                        onClick={onCancel}
                ...
                <div className='p-6 space-y-8'>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-8'>
                            <div className='space-y-4'>
                                ...
                                <div className='grid grid-cols-4 sm:grid-cols-6 gap-2'>
                                    {PROJECT_ICONS.slice(0, showAllOptions ? undefined : INITIAL_OPTIONS_COUNT).map(icon => (
                                        <button
                                            key={icon}
                            ...
                            <div className='space-y-4'>
                                ...
                                <div className='grid grid-cols-4 sm:grid-cols-6 gap-2'>
                                    {PROJECT_COLORS.slice(0, showAllOptions ? undefined : INITIAL_OPTIONS_COUNT).map(color => (
                                        <button
                                            key={color}
                    ...
                    <div className='p-6 rounded-2xl bg-surface-elevated border-4 border-border-strong shadow-brutal-xs space-y-4'>
                        ...
                        <TagInput
                            ...
                            onChange={(tags) => setFormData(prev => ({ ...prev, project_tags: tags }))}
                        />
                    </div>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                            <div className='space-y-2'>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFormData(prev => ({ ...prev, goal_type: e.target.value as EditableProject['goal_type'] }))}
                                >
                    ...
                    <div className='flex items-center gap-4 pt-4'>
                        <button
                            ...
                            className='flex-1 flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-primary-highlight text-white text-lg font-black border-2 border-border-strong shadow-brutal btn-brutal disabled:opacity-50'
                        >
                            {isSaving ? <Loader2 className='w-6 h-6 animate-spin' /> : <Check className='w-6 h-6' />}
                            {isNew ? 'ESTABLISH PROJECT' : 'LOCK CHANGES'}
                        </button>
