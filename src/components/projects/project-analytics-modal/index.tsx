...
export function ProjectAnalyticsModal({
    ...
    const stats = useMemo(() => {
        ...
        const todayDuration = projectSessions
            .filter(s => (s.started_at || 0) >= todayStart)
            .reduce((acc, s) => acc + (s.duration || 0), 0);

        const weekDuration = projectSessions
            .filter(s => (s.started_at || 0) >= weekStart)
            .reduce((acc, s) => acc + (s.duration || 0), 0);

        const monthDuration = projectSessions
    ...
    return (
        <Dialog open={!!project} onOpenChange={(open) => !open && onClose()}>
            <DialogContent showCloseButton={false} className='sm:max-w-md p-0 overflow-hidden border-4 border-border-strong bg-surface rounded-3xl shadow-brutal'>
                <div className='relative p-6 border-b-4 border-border-strong flex items-center gap-4' style={{ backgroundColor: `${project.color}15` }}>
                    <div
                    ...
                    <div>
                        ...
                        <DialogDescription className='text-sm text-muted-foreground'>Project Analytics</DialogDescription>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {isLoading ? (
                        ...
                        <>
                            ...
                            <div className='pt-4 border-t-2 border-border/50'>
                                <button
                                    onClick={onClose}
                                    className='w-full py-3 rounded-xl bg-primary-highlight text-white font-bold border-2 border-border-strong shadow-brutal btn-brutal'
                                >
                                    Close Insights
                                </button>
                            </div>
