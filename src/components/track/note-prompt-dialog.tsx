...
export type NotePromptDialogProps = {
    ...
    onStopWithoutNote: () => Promise<void>;
    isStopping: boolean;
};
...
export function NotePromptDialog({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && !isStopping && onClose()}>
            <DialogContent className="sm:max-w-md bg-surface dark:bg-surface-dark border-2 border-border-strong dark:border-border-strong-dark shadow-brutal dark:shadow-brutal-dark">
                ...
                <div className="space-y-4 py-4">
                    <div className="flex flex-col gap-1">
                        <span className="text-sm font-medium text-muted-foreground">Stopping session:</span>
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="note" className="text-sm font-semibold text-foreground dark:text-foreground-dark">
                        ...
                        <textarea
                            ...
                            className={cn(
                                "w-full min-h-[120px] p-3 rounded-md border-2 border-border-strong dark:border-border-strong-dark bg-transparent text-foreground dark:text-foreground-dark focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none",
                ...
                <DialogFooter className="flex-col sm:flex-row gap-2 sm:justify-between">
                    <Button
                        variant="outline"
                        onClick={onStopWithoutNote}
                        disabled={isStopping}
                        className="border-2 border-border-strong dark:border-border-strong-dark shadow-brutal-xs btn-brutal bg-surface dark:bg-surface-dark text-foreground dark:text-foreground-dark hover:bg-surface-elevated dark:hover:bg-surface-elevated-dark w-full sm:w-auto"
                    >
                        Stop without note
                    </Button>
